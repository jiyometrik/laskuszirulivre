...
Long gasze kartezsyana, laskufesta medt \(x\) au \(y\) dekti mahaseyena mit laskuriso.
...
Vi bruk kakutropos \(y = f\left(x\right)\): imina ka \(y\) laskudvaima midt \(x\) i.
Per nis-razdai laskudvaima fu \(x\), vi kakuti
\(f\left(x\right) = ax^2 + bx + c\). \footnote{Na \it{nis-razdai} laskudvaima \(f\left(x\right)=ax^2+bx+c\), \(a\neq0\). Naze takk?}
Nayang, vi kakuti ens-razdai laskudvaima fu \(x\) mit \(f\left(x\right) = mx + c\).
...
Vi deki mahase al absolutna gviratai fu \(x\) koske \(x > 0\) midt \it{bizsyaunafras}: \(f\left(x\right) > 1\).
...
Per laskudvaima \(f\left(x\right) = x^2-4\), \(f\left(x\right)\geq-4\) prosta midt anse. De, vi deki hanu
